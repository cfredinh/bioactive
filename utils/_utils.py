...
def create_umap_embeddings_no_labels(feature_bank, label_mapper=None, umap_path="umap_emb"):
    
    # create umaps and plot
    embedding = umap.UMAP().fit(feature_bank)

    fig_ = sns.scatterplot(x=embedding.embedding_[:,0],y= embedding.embedding_[:,1], s = 0.5)
    fi = fig_.get_figure()
    fi.savefig(umap_path, dpi=300)
...
class MixedLRScheduler():
    ...
    def __init__(self, schedulers, scheduler_types, steps_per_epoch):
        ...
        self.warmup_based    = ["LinearWarmup"]
        ...
        self.warmup_iters = [self.schedulers[sc].warmup_iters 
                             ...
                             if sctype in self.warmup_based]
...
class HookedModel(nn.Module):
    def __init__(self, model, layer_names):
        ...
        layer_mod_dict = OrderedDict([*self.model.named_modules()])
        self.outputs = OrderedDict([(_l,None) for _l in self.layer_names])
        
        for layer_name in self.layer_names:
            layer = layer_mod_dict[layer_name]
            layer.register_forward_hook(self.hook_fn(layer_name))
            

    def hook_fn(self, layer_name):
        def hook_app(_, __, output):
            self.outputs[layer_name] = output.clone().detach().cpu()
        return hook_app
...
def shift_colors(img):
    ...
    power_off = np.clip(np.random.normal(1.0,0.05,size=(1,1,shape_)), 0.95, 1.05)
    img = np.power(img+1.0, power_off)
    return img
