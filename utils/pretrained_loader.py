...
def load_from_pretrained(model, path, strict=False, transfer_learning_params=None):
    """
    Loads wights to the input model from a pretrained model path
    """
    pretrained_state = state_dict_from_path(path)
    if (transfer_learning_params != None) and ("load_full_model" in transfer_learning_params) and (transfer_learning_params["load_full_model"]):
