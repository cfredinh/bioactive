...
class DefaultWrapper:
    ...
    def init_dataloaders(self, collate_fn=None) -> edict:
        ...
        self.dataset_params.ssl_mode = False if self.is_supervised else True
        ...
        self.is_masked_multilabel = trainset.is_masked_multilabel      
    ...
    def init_model(self) -> Classifier:
        ...
        if self.transfer_learning_params.use_pretrained:
            ...
            load_from_pretrained(model, pretrained_path, strict=True, transfer_learning_params = self.transfer_learning_params)    
    ...
    def init_criteria(self):          
        ...
        if self.task == 'classification':
            ...
            elif self.is_masked_multilabel:
                print("Getting BCEMASKEDLoss")
                crit = BCEMASKEDLoss()
            else:
                crit = nn.BCEWithLogitsLoss()
                print_ddp(f"Default criterion: \033[94mBCEWithLogitsLoss\033[0m")
        else:
    ...
    def init_metrics(self):
        if self.task == 'classification':
            if self.is_masked_multilabel:
                self.metric = MultiLabelMaskClassificationMetrics
        else:
            raise NotImplementedError("Only classification tasks are implemented for now")    
