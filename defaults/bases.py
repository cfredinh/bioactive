...
class BaseSet(Dataset):
    ...
    def __getitem__(self, idx): 
        ...
        if isinstance(img_path, list):
            img = np.array([cv2.imread(impath, cv2.IMREAD_GRAYSCALE) for impath in img_path]).T
            assert self.img_channels == img.shape[-1], \
            f"cv2 shape must be WHC but C={img.shape[-1]} instead of {self.img_channels}"
        else:
    ...
    def get_trans_list_albumentations(self, transform_dict):
        ...
        if "Resize" in transform_dict:
            if transform_dict['Resize']['apply']:
                transform_list.append(A.Resize(height=transform_dict['Resize']['height'],
                                             width=transform_dict['Resize']['width']))

        if "CenterCrop" in transform_dict:                
            if transform_dict['CenterCrop']['apply']:
                transform_list.append(A.CenterCrop(height=transform_dict['CenterCrop']['height'],
                                                 width=transform_dict['CenterCrop']['width']))   

        if "RandomCrop" in transform_dict:                
            if transform_dict['RandomCrop']['apply']:
                padding = transform_dict['RandomCrop']['padding']
                transform_list.append(A.RandomCrop(height=transform_dict['RandomCrop']['height'],
                                             width=transform_dict['RandomCrop']['width']))
                print("WARNING: padding not included in albumentations implementation")

        if "RandomResizedCrop" in transform_dict:                
            if transform_dict['RandomResizedCrop']['apply']:
                transform_list.append(A.RandomResizedCrop(height=transform_dict['RandomResizedCrop']['size'],
        ...
        if "VerticalFlip" in transform_dict:                
            if transform_dict['VerticalFlip']['apply']:
                transform_list.append(A.VerticalFlip(p=transform_dict['VerticalFlip']['p']))

        if "HorizontalFlip" in transform_dict:                
            if transform_dict['HorizontalFlip']['apply']:
                transform_list.append(A.HorizontalFlip(p=transform_dict['HorizontalFlip']['p']))

        if "RandomRotation" in transform_dict:                
            if transform_dict['RandomRotation']['apply']:
                transform_list.append( A.Rotate(limit=transform_dict['RandomRotation']['angle'],
                                                      p=transform_dict['RandomRotation']['p']))

        if "RandomRotate90" in transform_dict:                
            if transform_dict['RandomRotate90']['apply']:
                transform_list.append( A.RandomRotate90(p=transform_dict['RandomRotate90']['p']))
        
        
        if "RandomBrightnessContrast" in transform_dict:                
            if transform_dict['RandomBrightnessContrast']['apply']:
                transform_list.append( A.RandomBrightnessContrast(p=transform_dict['RandomBrightnessContrast']['p']))
                
        
        if "RandomGamma" in transform_dict:                
            if transform_dict['RandomGamma']['apply']:
                transform_list.append( A.RandomGamma(p=transform_dict['RandomGamma']['p']))
        
                
        if "ColorJitter" in transform_dict:                
            if transform_dict['ColorJitter']['apply']:
                temp_d = transform_dict['ColorJitter']
                transform_list.append(A.HueSaturationValue(hue_shift_limit=temp_d['hue'], sat_shift_limit=temp_d['saturation'], val_shift_limit=temp_d['brightness'], p=temp_d['p']))

        if "RandomGrayscale" in transform_dict:                
            if transform_dict['RandomGrayscale']['apply']:
                transform_list.append(A.ToGray(p=transform_dict['RandomGrayscale']['p']))             

        if "GaussianBlur" in transform_dict:                
            if transform_dict['GaussianBlur']['apply']:
                transform_list.append(A.GaussianBlur(p=transform_dict['GaussianBlur']['p']))
        ...
        if "Normalize" in transform_dict:            
            if transform_dict['Normalize']:       
                print(" Setting the normalization params: ", self.mean, self.std, self.max_pixel_value)         
                transform_list.append(A.Normalize(mean=self.mean, 
                                                std=self.std,
    ...
    def has_multi_crop(self, albumentations = False):        
        ...
        if "MultiCrop" in aplied_transforms:
            ...
            self.multi_crop_aug = MultiCrop(n_crops=aplied_transforms["MultiCrop"]["n_crops"],
                                                    ...
                                                    scales=aplied_transforms["MultiCrop"]["scales"], albumentations=albumentations)
        else:
            self.is_multi_crop = False

    def get_transforms(self, albumentations=False):   

        self.has_multi_crop(albumentations=albumentations) # Checking and init MultiCrop strategy
        aplied_transforms = self.get_transform_defs()
        ...
        if albumentations:
            if isinstance(aplied_transforms, list):
                transforms = [A.Compose(self.get_trans_list_albumentations(tr)) for tr in aplied_transforms]
            elif isinstance(aplied_transforms, dict):
                transforms = A.Compose(self.get_trans_list_albumentations(aplied_transforms))
            else:
                raise TypeError("Transform data structure not understood")
            
            return self.__class__.disentangle_resizes_from_transforms(transforms) # transforms, 1 #
    ...
    def disentangle_resizes_from_transforms(transforms):
        ...
        if isinstance(transforms, Compose):
            ...
            resize_free_trans = BaseSet.remove_transform_albumentations(resize_free_trans, A.Resize)
            resizing = None if not resizing else resizing[0]
            return resize_free_trans, resizing
